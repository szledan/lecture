...
class Panel {
  ...
  // private:
  html_id = "";
  settings = structuredClone(Panel.DEFAULT_SETTINGS);
  app = null;

  // public:
  constructor(html_id, app, settings) {
    const panelDiv = document.getElementById(html_id);
    if (panelDiv === null) {
      console.warn(`Cannot find '${html_id}' in html document!`);
      return;
    }
    this.html_id = html_id;

    /*if (app === null) {
      console.warn(`Undefined 'app'!`);
      return;
    }*/
    this.app = app;
    ...
    if (settings) {
      for (const key in this.settings) {
        if (settings && key in settings) {
          this.settings[key] = structuredClone(settings[key]);
        }
      }
    }
